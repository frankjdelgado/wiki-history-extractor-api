...
echo "********************** CREATE USER **********************"
docker-compose exec mongo mongo --eval "db=db.getSiblingDB('wiki_history_extractor_test'); db.createUser({ user: 'wiki', pwd: 'wiki123', roles: [{ role: 'readWrite', db: 'wiki_history_extractor_test' }, { role: 'read', db: 'local' }]});"
